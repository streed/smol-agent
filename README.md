...
Unstable **`session/set_model`** (SDK: `unstable_setSessionModel`) switches the LLM for an idle session via the same provider the process was started with; it cannot swap provider backends without restarting.
