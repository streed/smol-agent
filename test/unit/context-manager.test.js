...
function assertValidOrder(messages, label) {
  for (let i = 1; i < messages.length; i++) {
    if (messages[i].role === 'tool') {
      ...
      assertTrue(ok, `${label}: tool at index ${i} preceded by '${prev.role}' (tool_calls: ${!!prev.tool_calls})`);
    }
  }
}

export default async function runContextManagerTests() {
  await describe('ContextManager — message ordering after pruning', async () => {

    await test('pruneMessages drops orphaned tool messages', async () => {
    ...
    await test('handleOverflow produces valid ordering', async () => {
    ...
    await test('summarizeOldMessages produces valid ordering', async () => {
    ...
    await test('valid sequences are preserved unchanged', async () => {
      ...
      assertEqual(pruned, 0, 'Should not prune when under budget');
      assertEqual(result.length, messages.length, 'All messages preserved');
    });
  });
}
