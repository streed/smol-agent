...
export const MAX_LINT_ROUNDS = 2; // Stripe caps at 2 CI rounds — we do the same for lint
 ...
 */
export function detectLintCommands(cwd: string): LintCommand[] {
 ...
 */
function runLintCommand(cmd: LintCommand, cwd: string): Promise<LintResult> {
  return new Promise((resolve) => {
    exec(cmd.command, { cwd, timeout: cmd.timeout, encoding: "utf-8", maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
      const output = ((stdout || "") + "\n" + (stderr || "")).trim();
...
export class ShiftLeftFeedback {
  private cwd: string;
  private lintRounds: number;
  private lintCommands: LintCommand[] | null;
  private _lastModifiedFiles: Set<string>;
   ...
   */
  async runLint(): Promise<LintFeedback | null> {
