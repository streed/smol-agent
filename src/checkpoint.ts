 ...
 */
function checkpointRepoPath(cwd: string): string {
 ...
 */
function ensureCheckpointRepo(cwd: string): string {
 ...
 */
function getFilesToCheckpoint(cwd: string): string[] {
  try {
 ...
 */
function getWorkingTreeChanges(cwd: string): string[] {
  try {
 ...
 */
function syncToCheckpointRepo(cwd: string, repoPath: string): number {
 ...
 *
 * @param cwd - Working directory (must be a git repo)
 * @param label - Optional label for the checkpoint
 * @returns Result with created status and message or error
 */
export function createCheckpoint(cwd: string, label = ""): CheckpointResult {
  if (!isGitRepo(cwd)) {
  ...
  try {
    ...
    logger.warn(`Checkpoint creation failed: ${error.message}`);
 ...
 * @param limit - Max checkpoints to return
 * @returns Array of checkpoint info
 */
export function listCheckpoints(cwd: string, limit = 10): CheckpointInfo[] {
  const repoPath = checkpointRepoPath(cwd);
 ...
 *
 * @param cwd - Working directory
 * @param commitHash - Specific commit hash to restore (default: most recent checkpoint)
 * @returns Result with restored status and message or error
 */
export function rollbackToCheckpoint(cwd: string, commitHash?: string | null): RollbackResult {
  if (!isGitRepo(cwd)) {
  ...
  try {
    ...
    const error = err as Error;
    logger.warn(`Rollback failed: ${error.message}`);
 ...
 *
 * @param cwd - Working directory
 * @returns True if at least one checkpoint exists
 */
export function hasCheckpoint(cwd: string): boolean {
  return listCheckpoints(cwd, 1).length > 0;
}
 ...
 *
 * @param cwd - Working directory
 * @param _keep - Number of checkpoints to keep (unused, kept for API compat)
 */
export function cleanupCheckpoints(cwd: string, _keep = 5): void {
  const repoPath = checkpointRepoPath(cwd);
  if (!fs.existsSync(path.join(repoPath, ".git"))) return;
