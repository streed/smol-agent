 ...
 * @param {string} [branch] - Branch to find the base for (default: current HEAD)
 * @returns {string} The merge-base ref to diff against
 */
function detectBaseBranch(cwd, branch) {
 ...
 * @param {string} [branch] - Branch to review (default: current branch)
 * @returns {{ diff: string, log: string, uncommitted: string, branch: string }} The git context
 */
function gatherGitChanges(cwd, branch) {
 ...
 *
 * @param {object} clientOrProvider - Ollama client or BaseLLMProvider instance
 * @param {string|object} modelOrOptions - Model name (legacy) or options (provider mode)
 ...
 * @param {string} [options.branch] - Branch to review (default: current branch)
 * @returns {Promise<string>} The code review
 */
export async function reviewPass(clientOrProvider, modelOrOptions, options = {}) {
  ...
  if (typeof modelOrOptions === "string") {
    ...
    provider = new OllamaProvider({ model: modelOrOptions });
    provider.client = clientOrProvider;
    opts = options;
  } else {
    // New: reviewPass(provider, options)
    provider = clientOrProvider;
    opts = modelOrOptions || {};
  }
  ...
  for (let i = 0; i < MAX_REVIEW_ITERATIONS; i++) {
    ...
    if (toolCalls.length === 0 && content) {
      toolCalls = parseToolCallsFromContent(content)
        .filter(tc => REVIEW_TOOLS.has(tc.function.name));
    }
