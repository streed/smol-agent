 ...
 *
 * @file-doc
 * @module tools/run_command
 * @dependencies node:child_process, ./registry.js, ../path-utils.js
 * @dependents src/acp-server.js, src/agent.js, src/tools/code_execution.js,
...
register("run_command", {
  ...
  async execute({ command, cwd: argCwd, timeout }: RunCommandArgs, { cwd: baseCwd = process.cwd() } = {}): Promise<RunCommandResult> {
    ...
    return new Promise((resolve) => {
      exec(
        ...
        (err: Error | null, stdout: string, stderr: string) => {
          if (err) {
            const execErr = err as Error & { code?: number };
            resolve({
